...
def up(force, env=None, **kwargs):
    network = en.G5kNetworkConf(type="prod", roles=["my_network"], site="lille")

    conf = (
        en.G5kConf.from_settings(job_type="exotic", job_name="EnosLib FTW", walltime="00:30:00")
            .add_network_conf(network)
            .add_machine(
            roles=["builder"], cluster="chiclet", nodes=1, primary_network=network
        )
            .finalize()
    )
