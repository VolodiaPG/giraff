...
for ID in $(seq 1 "${nb_instances}"); do
  ...
  if [[ $ID -eq 1 ]]; then
      ...
      SLEEP=$(( 1 * ID ))
      tmux split-window -d -t my_session_name:0 -p20 -v "printf '\033]2;%s\033\\' '${ID} — $(cat node-situation-${ID}.ron | sed -n 's/\s*my_id\s*:\s*\"\(.*\)\"/ \1 /p')' ; sleep ${SLEEP} ; source start_node.sh ${ID}; echo 'Waiting for keypress to close...' && read" #-n "totot" #"#${ID} — $(cat node-situation-${ID}.ron | sed -n 's/\s*my_id\s*:\s*\"\(.*\)\"/ \1 /p')"
  fi
done
