...
cd <- cachem::cache_disk(
  rappdirs::user_cache_dir("R-giraff"),
  max_size = 20 * 1024^3
)
...
combine2 <- memoise2(
  function(arks, cb_name, cb) {
    Log(paste0("Combining ", cb_name))
    tmp <- foreach(
      ark = arks,
      .verbose = FALSE,
      .packages = c("purrr", "dplyr", "multidplyr"),
      .combine = bind_rows
    ) %dopar%
      mem(cb)(ark)
...
if (single_graphs) {
  ...
  graphs <- foreach(
  ...
  ) %dopar%
    {
      ...
      graphs <- graph_non_ggplot(
        "respected_sla",
        graphs,
        output_respected_sla_plot(respected_sla, bids_won_function, node_levels)
      )
      graphs <- graph_non_ggplot(
        "sla",
        graphs,
        output_sla_plot(respected_sla, bids_won_function, node_levels)
      )

      graphs <- graph(
        "duration_distribution",
        graphs,
        output_duration_distribution_plot(provisioned_sla)
      )
      graphs <- graph(
        "latency_distribution",
        graphs,
        output_latency_distribution_plot(provisioned_sla)
      )
      graphs <- graph(
        "request_interval_distribution",
        graphs,
        output_request_interval_distribution_plot(provisioned_sla)
      )
      graphs <- graph(
        "request_distribution",
        graphs,
        output_request_distribution(respected_sla)
      )
      graphs <- graph(
        "latency_vs_expected_latency",
        graphs,
        output_latency_vs_expected_latency_plot(
          respected_sla,
          bids_won_function
        )
      )
      graphs <- graph(
        "in_flight_time",
        graphs,
        output_in_flight_time_plot_simple(
          respected_sla,
          bids_won_function,
          node_levels
        )
      ...
      graphs <- graph(
        "ran_for",
        graphs,
        output_ran_for_plot_simple(respected_sla, bids_won_function)
      )
      ...
      graphs <- graph(
        "spending",
        graphs,
        output_spending_plot_simple(bids_won_function, node_levels)
      )
      graphs <- graph(
        "faults_per_function",
        graphs,
        output_faults_per_function_plot_simple(respected_sla)
      )
...
earnings_jains_plot_data <- mem(load_earnings_jains_plot_data)(
  node_levels,
  bids_won_function
)
...
# graph_spider_chart <- export_graph(
#   "output_spider_chart",
#   output_placement_method_comparison(
#     respected_sla,
#     functions_total,
#     node_levels,
#     bids_won_function,
...
#   )
# )
graph_output_mean_respected_slas <- wrap_graph(
  output_mean_respected_slas(respected_sla, node_levels)
)
graph_output_mean_deployment_time <- wrap_graph(
  output_mean_deployment_times(raw_deployment_times, node_levels, respected_sla)
)
graph_output_mean_spending <- wrap_graph(
  output_mean_spending(bids_won_function, node_levels, respected_sla)
)
graph_output_mean_placed_functions_per_node <- wrap_graph(
  output_mean_placed_functions_per_node(provisioned_functions, node_levels)
)
graph_output_mean_latency <- wrap_graph(
...
cat(
  "Maximum number of functions hosted on a single node:",
