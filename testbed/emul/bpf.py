...
class HTBConstraint:
    ...
    def __post_init__(self):
        HTBConcreteConstraintValidator.validate(
            dict(device=self.device, target=self.target, rate=self.rate)
        )
    ...
    def remove_commands(self) -> List[str]:
        """Remove everything."""
        cmds = [
            f"tc qdisc del dev {self.device} clsact || true",
            f"tc qdisc del dev {self.device} root || true",
            "rm /sys/fs/bpf/IP_HANDLE_BPS_DELAY || true",
            "rm /sys/fs/bpf/progs || true",
        ]
        return cmds
